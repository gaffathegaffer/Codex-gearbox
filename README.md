...
Each new run stores `.gearbox/runs/<run-id>/run-state.json` and `events.jsonl`. The state is versioned (`gearbox.run-state`, version 1), validated in a same-directory temporary file and replaced atomically on Windows, and records the task, profile, tier/reasoning, checkpoint, worker history, verification, review, and terminal status. Events are compact JSON Lines intended for manual diagnosis; append failures are warned about without aborting the run, and inspection tolerates corrupt trailing event lines.

Use the installed or checkout entry point with `-InspectRun <run-id>` to inspect a run without starting a worker. Use `-Resume <run-id>` to continue only from a safe persisted checkpoint. Resume refuses terminal runs, corrupt or unknown state versions, missing workdirs, changed Git root/HEAD/branch/status, changed non-Git top-level file fingerprint, a `worker_started` checkpoint whose outcome is unknown, and `after_worker_result` or `before_review` checkpoints whose persisted decision is not replayable. This conservative refusal avoids rerunning or skipping a worker when the result was persisted but the router decision was not durably replayed.
...
Worker preflight validates that the selected Codex executable belongs to a sandbox-ready distribution with adjacent `codex-windows-sandbox-setup.exe`, `codex-command-runner.exe`, and `codex-code-mode-host.exe`. An explicit valid `CODEX_CLI_PATH` remains authoritative, but an incomplete distribution is refused before model execution; Gearbox never assembles a worker from binaries in different distributions. Without an explicit path, discovery prefers complete user-owned AppData/plugin distributions and ignores WindowsApps candidates that are not sandbox-ready. `shell-preflight.json` records distribution root, readiness, companions, missing files, and the separate child-PATH WindowsApps filter. The recommended complete distribution on this machine is `C:\Users\Gaffa\AppData\Local\OpenAI\Codex\bin\d7e8094cfb76a267\codex.exe`; `C:\Users\Gaffa\.codex\.sandbox-bin\codex.exe` is incomplete and is not a worker CLI.
